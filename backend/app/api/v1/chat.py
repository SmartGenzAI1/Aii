...
async def chat(
    ...
    if user.daily_used >= user.daily_quota:
        ...
        raise HTTPException(
            status_code=429,
            detail=f"Daily quota exceeded ({user.daily_used}/{user.daily_quota})",
        )

    # ===== MODEL RESOLUTION =====
    try:
        provider, real_model = resolve_model(payload.model)
        logger.info(f"Model resolved: {payload.model} -> {provider}/{real_model}")
    except KeyError:
    ...
    try:
        ...
        logger.info(
            f"Chat started for {user.email} "
            f"(provider: {provider}, model: {payload.model})"
        )

        return stream_response(stream_generator)

    except Exception as e:
        logger.error(f"AI routing failed: {str(e)}", exc_info=True)

        # Check if it's a known error
        if "rate limit" in str(e).lower():
            raise HTTPException(
                status_code=429,
                detail="AI provider rate limited. Try again later.",
            )
...
async def get_quota(
    ...
    return {
        ...
        "resets_at": user.last_reset.isoformat(),
    }
